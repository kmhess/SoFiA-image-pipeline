...
def get_noise_spec(source, src_basename, cube_params, original=None, overwrite=False):
    ...
    if not os.path.isfile(outfile):
        ...
        try:
            ...
            # Change 'col#' to actually column name:
            for old_col, new_col in zip(spec_template.colnames, col_names):
                spec_template.rename_column(old_col, new_col)
...
def make_spec_aper(source, src_basename, cube_params, original, spec_line=None, suffix='png', both=False,
    ...
    if not os.path.isfile(outfile2):
        ...
        try:
            ...
            if 'freq' in source.colnames:
                ...
                for old_col, new_col in zip(spec.colnames, col_names[:len(spec.colnames)]):
                    spec.rename_column(old_col, new_col)
                optical_velocity = (source['freq'] - spec['freq'])/spec['freq'] * const.c.to(u.km/u.s).value
                if both == True:
                    spec2 = ascii.read(specfile2)
                    for old_col, new_col in zip(spec2.colnames, col_names[:len(spec2.colnames)]):
                        spec2.rename_column(old_col, new_col)
        ...
        if 'freq' in source.colnames:
            ...
            ax2b_spec.set_xlabel('{} Frequency [MHz]'.format(cube_params['spec_sys']), fontsize=17)
            if freq1 >= 2*u.GHz:
                freq1 = freq1.to(u.GHz)
                freq2 = freq2.to(u.GHz)
                ax2b_spec.set_xlabel('{} Frequency [GHz]'.format(cube_params['spec_sys']), fontsize=17)
...
def make_spec(source, src_basename, cube_params, spec_line=None, suffix='png', id_label='', overwrite=False):
    ...
    if not os.path.isfile(outfile1):
        ...
        if 'freq' in source.colnames:
            ...
            freq1 = (spec['freq'][-1] * u.Hz).to(u.MHz)
            freq2 = (spec['freq'][0] * u.Hz).to(u.MHz)
            ax1b_spec.set_xlabel('{} Frequency [MHz]'.format(cube_params['spec_sys']), fontsize=17)
            if freq1 >= 2*u.GHz:
                freq1 = freq1.to(u.GHz)
                freq2 = freq2.to(u.GHz)
                ax1b_spec.set_xlabel('{} Frequency [GHz]'.format(cube_params['spec_sys']), fontsize=17)
